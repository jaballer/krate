...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($requestHelper->post('new_password'))) {
            if ($requestHelper->post('new_password') !== $requestHelper->post('confirm_password')) {
                throw new InvalidArgumentException("New passwords do not match.");
            }
            
            if (!$userManager->changePassword($user_id, $requestHelper->post('current_password'), $requestHelper->post('new_password'))) {
        ...
        if (!empty($requestHelper->post('first_name')) && !empty($requestHelper->post('last_name')) && !empty($requestHelper->post('email'))) {
            if ($userManager->updateUserProfile($user_id, [
                'first_name' => $requestHelper->post('first_name'),
                'last_name' => $requestHelper->post('last_name'),
