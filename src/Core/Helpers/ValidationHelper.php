...
class ValidationHelper
{
    ...
    public static function has_length_greater_than($value, $min) {
        $length = strlen($value);
        return $length > $min;
    }
    ...
    // * spaces count towards length
    // * use trim() if spaces should not count
    public static function has_length_less_than($value, $max) {
        $length = strlen($value);
        return $length < $max;
    }
    ...
    // * spaces count towards length
    // * use trim() if spaces should not count
    public static function has_length_exactly($value, $exact) {
        $length = strlen($value);
        return $length == $exact;
    }
  ...
  public static function has_inclusion_of($value, $set) {
  	return in_array($value, $set);
  }

  // has_exclusion_of( 5, [1,3,5,7,9] )
  // * validate exclusion from a set
  public static function has_exclusion_of($value, $set) {
    return !in_array($value, $set);
  }
  ...
  public static function has_string($value, $required_string) {
  ...
  public static function has_unique_page_menu_name($menu_name, $current_id = "0", DatabaseConnection $db) {
    $sql = "SELECT * FROM pages ";
    $sql .= "WHERE menu_name='" . $db->escape($menu_name) . "' ";
    $sql .= "AND id != '" . $db->escape($current_id) . "'";

    $page_set = $db->query($sql);
    $page_count = $db->num_rows($page_set);
    $db->free_result($page_set);
  ...
  public static function has_unique_username($username, $current_id="0") {
    global $db;
