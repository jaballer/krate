...
class UserManager
{
    /** @var string Fallback "from" address used when MAIL_FROM_ADDRESS is unset */
    private const DEFAULT_FROM_EMAIL = 'no-reply@localhost';

    /** @var array<string> Fallback admin recipients used when ADMIN_NOTIFICATION_EMAILS is unset */
    ...
    private function sendLoginNotificationToAdmins(string $first_name, string $email): void
    {
        ...
        foreach ($this->getAdminEmails() as $toEmail) {
            try {
    ...
    /**
     * Resolves the "from" address for notification emails.
     *
     * Reads MAIL_FROM_ADDRESS from the environment, falling back to the
    ...
    private function getFromEmail(): string
    {
        ...
        return $fromEmail !== '' ? $fromEmail : self::DEFAULT_FROM_EMAIL;
    }

    /**
     * Resolves the list of admin recipients for notification emails.
     *
     * Reads ADMIN_NOTIFICATION_EMAILS (a comma-separated list) from the
     * environment, falling back to the defaults when it is unset or empty.
     *
     * @return array<string>
     */
    private function getAdminEmails(): array
    {
        $raw = trim($_ENV['ADMIN_NOTIFICATION_EMAILS'] ?? '');

        if ($raw === '') {
            return self::DEFAULT_ADMIN_EMAILS;
        }

        $emails = array_filter(array_map('trim', explode(',', $raw)));

        return $emails !== [] ? array_values($emails) : self::DEFAULT_ADMIN_EMAILS;
    }
